...
export default function DashboardClient({ initialStudents, initialTextbooks }: Props) {
  const supabase = createClient()
  const router = useRouter()
  const [students] = useState<Student[]>(initialStudents)
  const [textbooks] = useState<Textbook[]>(initialTextbooks)
  ...
  async function selectStudent(s: Student) {
    ...
    setForm(f => ({ ...f, nextDate: getNextLesson(s.lesson_days ?? [], s.lesson_time) }))
  ...
  function generateMessage() {
    ...
    if (assignType === 'textbook') {
      ...
      setPendingAssign({ student_id: selected.id, textbook_id: form.newTextbook, assign_type: 'textbook', start_page: parseInt(form.newStart), end_page: parseInt(form.newEnd) })
    } else {
      ...
      setPendingAssign({ student_id: selected.id, textbook_id: form.newExam, assign_type: 'exam', start_item: parseInt(form.newExamStart), end_item: parseInt(form.newExamEnd) })
    }
  ...
  const s = {
    ...
    sBtn: (sel: boolean) => ({ background: sel ? 'rgba(124,108,248,0.1)' : 'var(--surface2)', border: `2px solid ${sel ? 'var(--accent)' : 'var(--border)'}`, borderRadius: '10px', padding: '14px 12px', cursor: 'pointer', textAlign: 'left' as const }),
    ...
    btnCopy: (c: boolean) => ({ width: '100%', background: c ? 'var(--accent)' : 'var(--accent2)', color: c ? 'white' : 'var(--bg)', border: 'none', borderRadius: '8px', padding: '10px', fontSize: '13px', fontWeight: 700, cursor: 'pointer', marginTop: '12px' }),
    preview: (hasMsg: boolean) => ({ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '18px', fontSize: '13px', lineHeight: 1.9, whiteSpace: 'pre-wrap' as const, minHeight: '160px', color: hasMsg ? 'var(--text)' : 'var(--text3)', display: 'flex', alignItems: hasMsg ? 'flex-start' : 'center', justifyContent: hasMsg ? 'flex-start' : 'center' }),
    statusBadge: (st: string) => {
      ...
      const [bg, color] = map[st] ?? ['var(--surface2)', 'var(--text2)']
    ...
    logoutBtn: { background: 'transparent', border: '1.5px solid var(--border)', color: 'var(--text2)', borderRadius: '6px', padding: '6px 14px', fontSize: '12px', cursor: 'pointer' },
    prevBox: { background: 'rgba(86,207,178,0.06)', border: '1px solid rgba(86,207,178,0.2)', borderRadius: '8px', padding: '14px', marginBottom: '14px' },
    prevLabel: { fontSize: '11px', fontWeight: 700, color: 'var(--accent2)', marginBottom: '10px' },
    row2: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '14px' },
    rangeRow: { display: 'flex', gap: '6px', alignItems: 'center' },
  }
    ...
    <div style={s.page}>
      ...
      <div style={s.app}>
        ...
        <div style={s.card}>
          ...
          <div style={s.body}>
            {students.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--text3)', padding: '20px' }}>학생 데이터가 없습니다</div>
            ) : (
              <div style={s.grid}>
                {students.map(st => (
        ...
        <div style={s.card}>
          ...
          <div style={s.body}>
            ...
            {assignType === 'textbook' ? (
              <>
                <div style={s.prevBox}>
                  <div style={s.prevLabel}>✅ 완료된 범위 (선택)</div>
                  <div style={s.row2}>
              ...
              <>
                <div style={s.prevBox}>
                  <div style={s.prevLabel}>✅ 완료된 기출 (선택)</div>
                  <div style={s.row2}>
        ...
        <div style={s.card}>
          ...
          <div style={s.body}>
            <div style={s.preview(!!message)}>
              {message || '학생을 선택하고 과제를 입력하면\n메시지가 자동 생성됩니다'}
            </div>
            {message && (
              <>
                <button style={s.btnCopy(copied)} onClick={copyMessage}>{copied ? '✅ 복사됨!' : '📋 메시지 복사'}</button>
